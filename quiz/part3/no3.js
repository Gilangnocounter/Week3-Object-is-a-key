...
function countProfit(shoppers) {
    ...
    for(barang of listBarang){
        ...
        let profit = 0;
        for(customer of shoppers){
            if(customer.product === barang[0] && customer.amount<=barang[2]){
                data.shoppers.push(customer.name);
                barang[2] -= customer.amount; //update stock pada list barang
                profit += customer.amount*barang[1];    
            } 
        }
        data.leftOver = barang[2]
